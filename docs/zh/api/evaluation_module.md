...
|--|--|--|--|
|answer_correctness|通过结合事实准确性和语义相似度，来衡量模型答案与标准答案之间的正确性。|[user_input, response, reference]|范围：[0, 1]，值越大越正确。该指标关注两方面：回答是否与事实相符（事实准确性）以及回答的表达是否与标准答案语义相近（语义相似度）只有既保证内容真实可靠，又能与参考答案意思相匹配，才算“正确”。得分越高，说明模型输出的答案既真实又语义吻合。|
