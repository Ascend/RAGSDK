...
在原始RAG SDK流程中，在知识文档检索前，增加问答Cache，如果查询命中了Cache，就不会经过大模型推理过程，节省了知识文档检索和大模型推理时延，提升了端到端性能，经过性能测试，缓存命中可以大幅度提升端到端性能。
...
|**kwargs|Dict[str, Any]|必选|具体介绍可参见[create_embedding](./embedding.md#create_embedding)。|
...
|reverse|bool|可选|相似度分数和相似度的关系，默认值为"False"。<li>False：相似度分数越高，相似度越高。</li><li>True：相似度分数越高，相似度越低。</li>|
...
|参数名|数据类型|可选/必选|说明|
|--|--|--|--|
|verbose|bool|必选|是否开启详细日志记录，默认值为False。<li>True：记录cache过程的命中或者未命中。</li><li>False：不会记录cache过程的命中或者未命中。</li>|
...
|数据类型|说明|
|--|--|
|Union[Dict, Iterator[Dict]]|返回问答结果，其中Dict内容为：<li>带知识来源：{"query": query, "result": data, "source_documents": [{'metadata': xxx, 'page_content': xxx}]}</li><li>不带知识来源：{"query": query, "result": data}</li>|
...
|max_tokens|int|可选|用于截断正文的最大token大小，超出部分丢弃，取值范围[500, 100000]，默认值为1000。|
