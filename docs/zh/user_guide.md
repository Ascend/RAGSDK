...
1.  编译检索算子，以实现检索功能。
    ...
    >-   **-d** _<dim\>_表示embedding模型向量化后的维度，因acge\_text\_embedding嵌入模型向量维度为1024，这里设置为-d 1024。
...
>-   执行样例代码时，当参数<b>"tei\_emb"</b>为“False“，表示本地启动embedding模型，embedding\_path传入本地模型存放目录；当参数<b>"tei\_emb"</b>为“True“，表示启动服务化模型，embeddingurl传入服务化模型URI地址；reranker同理。
