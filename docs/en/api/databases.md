...
|--|--|--|--|
|engine|Engine|Required|An `Engine` instance. For details, see [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine). Only the OpenGauss dialect is supported.<br>>[!NOTE] The `Engine` instance comes from the user. Therefore, ensure that you use a secure connection method.|
...
Searches for document information in the relational database. If the `enable_bm25` parameter was set to `False` when creating the `OpenGaussDocstore` instance, this API is unavailable and returns an empty list.
...
|top_k|int|Optional|The maximum number of best-matching chunks to return. The default value is `3`. If this value is greater than the number of valid chunks retrieved, only the valid chunks are returned. The value range is `(0, 10000]`.|
...
|Data Type|Description|
|--|--|
|List[MxDocument]|Returns a list of `MxDocument` instances when results are found. Returns an empty list when no result is found. See [`MxDocument`](#mxdocument). If `enable_bm25` is `False`, the return value is `[]`.|

#### `drop`

**Description**

Drops the database managed by the current instance.
...
Provides a Milvus-based knowledge database that mainly stores split chunk information.

**Function Prototype**

```python
from mx_rag.storage.document_store import MilvusDocstore
MilvusDocstore(client, collection_name, enable_bm25, bm25_k1, bm25_b, auto_flush, encrypt_fn, decrypt_fn)
```
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|client|MilvusClient|Required|A `MilvusClient` instance. It supports [server mode](https://milvus.io/docs/connect-to-milvus-server.md) and [Lite mode](https://milvus.io/docs/milvus_lite.md).<br>> [!NOTE] The `MilvusClient` instance comes from the user. Therefore, ensure that you use a secure connection method.|
...
|bm25_k1|float|Optional|Controls term frequency saturation in BM25 sparse vector retrieval. A larger value increases the importance of term frequency in document ranking. The value range is [1.2, 2.0]. The default value is 1.2. For details, see the relevant section in [Milvus Full-Text Search](https://milvus.io/docs/full-text-search.md#Full-Text-Search).|
|bm25_b|float|Optional|Controls the degree of document length normalization in BM25 sparse vector retrieval. The value range is [0, 1]. The default value is 0.75. For details, see the relevant section in [Milvus Full-Text Search](https://milvus.io/docs/full-text-search.md#Full-Text-Search).|
...
|encrypt_fn|Callable[[str], str]|Optional|A callback that returns a string no longer than `128 * 1024 * 1024` characters. It takes effect when `enable_bm25` is `False`. When calling `add` or `update`, the document `page_content` is encrypted by `encrypt_fn` before being stored.<br>If the uploaded document involves personal data such as bank card numbers, ID card numbers, passport numbers, or passwords, configure this parameter to ensure data security.|
|decrypt_fn|Callable[[str], str]|Optional|A callback that returns a string no longer than `16 * 1024 * 1024` characters. It takes effect when `enable_bm25` is `False`. When calling query APIs, the `page_content` is decrypted by `decrypt_fn` before being returned.|
...
Saves document fragment information in the database.
...
Searches for text information in the database. The search uses BM25 sparse vector retrieval. If the `enable_bm25` parameter was set to `False` when creating the `MilvusDocstore` instance, this API is unavailable and returns an empty list.
...
|query|str|Required|The text to search. The length range is `(0, 1000 * 1000]`.|
|top_k|int|Optional|The maximum number of best-matching chunks to return. The default value is `3`. If this value is greater than the number of valid chunks retrieved, only the valid chunks are returned. The value range is `(0, 10000]`.|
|drop_ratio_search|float|Optional|The proportion of small vector values to exclude during BM25 sparse vector search. This option allows fine-tuning the search process by specifying the proportion of minimum values in the query vector to ignore. It helps balance search accuracy and performance. A smaller `drop_ratio_search` value reduces the contribution of these small values to the final score. By ignoring some small values, search performance can be improved while minimizing the impact on precision. The value range is [0, 1). The default value is 0.2. For details, see the relevant section in [Milvus Sparse Embeddings](https://milvus.io/docs/index.md?tab=sparse).|
|filter_dict|Dict|Required|A dictionary of search conditions. Currently, only filtering by `document_id` is supported. Pass the filtered document IDs as a list, and the list length cannot exceed `1000 * 1000`. For example, to filter among documents with `document_id` values 1, 2, and 4, pass `{"document_id": [1, 2, 4]}`.|
...
Drops the collection data of the instance's Milvus service.
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|db_path|str|Required|The relational database storage path. It must be a valid path. The maximum path length is 1024, the file name length cannot exceed 200, `..` is not allowed, and the path cannot be in any of the following locations: `["/etc", "/usr/bin", "/usr/lib", "/usr/lib64", "/sys/", "/dev/", "/sbin", "/tmp"]`.|
|encrypt_fn|Callable[[str], str]|Optional|A callback that returns a string no longer than `128 * 1024 * 1024` characters. It encrypts the `chunk` content of the [`ChunkModel` class](#chunkmodel-class) and returns a string. When `add` saves data, the database stores the data processed by `encrypt_fn` in the `chunk` field.<br>> [!NOTICE] If the uploaded document involves personal data such as bank card numbers, ID card numbers, passport numbers, or passwords, configure this parameter to ensure data security.|
|decrypt_fn|Callable[[str], str]|Optional|A callback that returns a string no longer than `16 * 1024 * 1024` characters. It decrypts the `chunk` content of the `ChunkModel` class and returns a string. When `search` returns data, it returns the data processed by `decrypt_fn` in the `chunk` field.|
...
Saves document fragment information in the relational database.
...
|x_dim|int|Optional|The vector dimension. This method calls `MilvusClient.create_collection`. See [create_collection](https://milvus.io/docs/index-vector-fields.md?tab=floating#Index-a-Collection).|
...
|params|dict|Optional|Additional parameters for the index type. The default value is `None`. If `None`, this field is set to an empty dictionary. See [milvus In-memory-Index](https://milvus.io/docs/index.md?tab=floating#In-memory-Index). The dictionary validation rules require that string lengths in the dictionary cannot exceed 1024 characters, iterable sequence lengths in the dictionary cannot exceed 1024, the dictionary length cannot exceed 1024, and the nested depth of the dictionary cannot exceed 2 levels. Add an extra top-level type key, `sparse` or `dense`, to indicate whether the parameters apply to dense or sparse retrieval. Example configuration: <br>{<br>"sparse": {},<br>"dense": {}<br>}|
