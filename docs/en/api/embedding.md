...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|kwargs|dict|Required|Keyword arguments. Refer to the input parameters of [TextEmbedding](#textembedding). Required parameters must be passed. Otherwise, a `KeyError` is raised.|
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|kwargs|dict|Required|Keyword arguments. Refer to the input parameters of [SparseEmbedding](#sparseembedding). Required parameters must be passed. Otherwise, a `KeyError` is raised.|
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|kwargs|dict|Required|Keyword arguments. Refer to the input parameters of [TEIEmbedding](#teiembedding). Required parameters must be passed. Otherwise, a `KeyError` is raised.|
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|kwargs|dict|Required|Keyword arguments. Refer to the input parameters of [CLIPEmbedding](#clipembedding). Required parameters must be passed. Otherwise, a `KeyError` is raised.|
...
|Data Type|Description|
|--|--|
|List[float]|The vector array converted from `text`. If the embedding model output is a 512-dimensional vector, the final output is a list of 512 floating point numbers.|
...
Starts the model locally with `cn_clip` and provide image and text embedding functionality. The `ImageEmbedding` class inherits from the `langchain_core.embeddings.Embeddings` interface.

> [!NOTE]
> `cn_clip` uses `torch.load` to load weight files. Ensure that the weight files are safe and trustworthy to prevent security issues such as command injection during weight loading.
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|model_name|str|Required|The model name. It must be one of `['ViT-B-16', 'ViT-L-14', 'ViT-L-14-336', 'ViT-H-14', 'RN50']`. For the corresponding model download links, see the webpage instructions.|
...
|dev_id|int|Optional|The NPU card ID where the model runs. Valid values are `[0, 63]`. The default is 0.|
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|kwargs|dict|Required|Keyword arguments. Refer to the input parameters of [ImageEmbedding](#imageembedding). Required parameters must be passed. Otherwise, a `KeyError` is raised.|
...
Converts a text list into vectors.
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|texts|List[str]|Required|A text list. The list length must be in the range `[1, 1000 * 1000]`. The length of each text must be in the range `[1, 256]`.|
|batch_size|int|Optional|The batch size. The method groups `batch_size` texts for each embedding operation. The value range is `[1, 1024]`. The default value is 32. The configurable value depends on the device memory.|
...
Converts a single text into a vector.
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|text|str|Required|The length range of the text is `[1, 256]`.|
...
Converts the given images into vectors.
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|images|Union[List[str], List[Image.Image]]|Required|When the input type is `List[str]`, each element is a base64-encoded string of an image. The total list length must be in the range `[1, 1000]`, and each element length must be in the range `[1, 10 * 1024 * 1024]`. When the input type is `List[Image.Image]`, the input data type is `PIL.Image.Image`.|
...
The factory method class for embeddings, used to produce embeddings for the RAG SDK.
...
# Modify the parameters based on the actual scenario
...
Constructs an embedding instance. This method calls the static `create` method of the `TextEmbedding`, `ImageEmbedding`, and `TEIEmbedding` classes to create class instances.
...
|Parameter|Data Type|Optional/Required|Description|
|--|--|--|--|
|embedding_type|str|Required|This parameter is passed in `kwargs` and specifies the type of embedding to create. It is passed as a keyword argument.<br>Valid values:<li>`local_text_embedding`</li><li>`local_images_embedding`</li><li>`tei_embedding`</li>|
|**kwargs|Any|Optional|Except for `embedding_type`, the remaining parameters are the construction parameters for the embedding. The corresponding class's static `create` method is called to return an instance.<li>If `local_text_embedding`, see [TextEmbedding](#textembedding).</li><li>If `local_images_embedding`, see [ImageEmbedding](#imageembedding).</li><li>If `tei_embedding`, see [TEIEmbedding](#teiembedding).</li>|
