...
def evaluate_creator(evaluator, evaluate_type: str):
    ...
    def evaluate_generate_relevancy(state):
        ...
        scores = evaluator.evaluate_scores(
            metrics_name=["answer_relevancy", "faithfulness"],
            datasets=datasets,
            language=language,
        )
...
def decide_to_decompose(state):
    ...
    if cache_generation is None:
        logger.warning("---DECISION: CACHE MISS GO DECOMPOSE---")
        return "cache_miss"
...
def grade_generation_v_documents_and_question(
    evaluate, context_score_threshold: float = 0.6, answer_score_threshold: float = 0.6
):
    ...
    def grade_generation_v_documents_and_question_process(state):
        ...
        if answer_score < answer_score_threshold:
            logger.warning(
                f"---DECISION: GENERATION DOES NOT ADDRESS QUESTION,"
        ...
        logger.info(
            f"---DECISION: GENERATION ADDRESSES QUESTION--- "
        ...
        if context_score < context_score_threshold:
            logger.warning(
                f"---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, "
        ...
        logger.info(
            f"---DECISION: GENERATION GROUNDED IN DOCUMENTS---"
...
def create_remote_connector(  # pylint: disable=redefined-outer-name
...
def create_knowledge_storage(  # pylint: disable=redefined-outer-name
    mxrag_component: Dict[str, Any], knowledge_files: List[str]
):
    ...
    vector_store = MindFAISS(x_dim=1024, devs=[npu_dev_id], load_local_index=faiss_index_save_file)
    ...
    upload_files(
        Knowledge_db,
        knowledge_files,
        loader_mng=mxrag_component.get("loader_mng"),
        embed_func=mxrag_component.get("embedding_connector").embed_documents,
        force=True,
    )


def create_hybrid_search_retriever(mxrag_component: Dict[str, Any]):  # pylint: disable=redefined-outer-name
    ...
    hybrid_retriever = EnsembleRetriever(retrievers=[npu_faiss_retriever], weights=[1.0])

    mxrag_component["retriever"] = hybrid_retriever


def create_cache(  # pylint: disable=redefined-outer-name
    mxrag_component: Dict[str, Any], reranker_url: str, embedding_url: str
):
...
def create_evaluate(mxrag_component):  # pylint: disable=redefined-outer-name
    from mx_rag.evaluate import RAGEvaluator

    llm = mxrag_component.get("llm_connector")
    embedding = mxrag_component.get("embedding_connector")
    mxrag_component["evaluator"] = RAGEvaluator(llm=llm, embeddings=embedding)


def build_mxrag_application(mxrag_component):  # pylint: disable=redefined-outer-name
    ...
    workflow.add_conditional_edges(
        ...
        {"useful": node_cache_update, "not useful": node_transform_query},
    )
