...
class TEISparseEmbeddingError(Exception):
    pass


class TEISparseEmbedding(Embeddings):
    ...
    def create(**kwargs):
        ...
        return TEISparseEmbedding(**kwargs)
    ...
    def embed_documents(self,
        ...
        for start_index in range(0, texts_len, batch_size):
            ...
            if not resp.success:
                raise TEISparseEmbeddingError("tei get response failed")
            ...
            try:
                ...
                data = [{int(key): value for key, value in sub_dict.items()} for sub_dict in data]
                result.extend(data)
            except json.JSONDecodeError as json_err:
                logger.error(f"Failed to parse JSON response for batch starting at {start_index}: {json_err}")
                raise TEISparseEmbeddingError(f"Unable to parse TEI response content: {json_err}") from json_err
                ...
                raise TEISparseEmbeddingError(f"TEI response data error: {data_err}") from data_err

            except Exception as e:
                logger.error(f"Unexpected error while processing batch starting at {start_index}: {e}")
                raise TEISparseEmbeddingError(
