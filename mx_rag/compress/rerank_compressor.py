...
from mx_rag.utils.common import validate_params, BOOL_TYPE_CHECK_TIP, MAX_PAGE_CONTENT, STR_TYPE_CHECK_TIP, TEXT_MAX_LEN


class RerankCompressor(PromptCompressor):
    @validate_params(
        reranker=dict(validator=lambda x: isinstance(x, Reranker), message="param must be instance of Reranker"),
        splitter=dict(
    ...
    def __init__(self, reranker: Reranker, splitter: TextSplitter = None):
    ...
    def compress_texts(self, context: str, question: str, compress_rate: float = 0.6, context_reorder: bool = False):
