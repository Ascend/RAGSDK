...
class Retriever(ABC):
    ...
    def __init__(self, vector_store, embed_func, k: int = 1, score_threshold: float = 0.1):
        super().__init__()
    ...
    def _get_relevant_documents(self, query: str) -> List[Doc]:
        sr: List[Tuple[Document, float]] = self._vector_store.similarity_search([query], self._embed_func, k=self._k)[0]
