...
from mx_rag.utils.common import validate_params, INT_32_MAX, STR_TYPE_CHECK_TIP
...
class TrainDataGenerator(BaseGenerator):
    @validate_params(
        ...
        document_path=dict(validator=lambda x: isinstance(x, str), message=STR_TYPE_CHECK_TIP),
        dataset_path=dict(validator=lambda x: isinstance(x, str), message=STR_TYPE_CHECK_TIP),
        embedding=dict(validator=lambda x: isinstance(x, str), message=STR_TYPE_CHECK_TIP),
        reranker=dict(validator=lambda x: isinstance(x, str), message=STR_TYPE_CHECK_TIP)
    )
    ...
    @validate_params(
        featured_percentage=dict(validator=lambda x: 0 <= x <= 1, message="param value length range [0, 1]"),
        llm_threshold_score=dict(validator=lambda x: 0 <= x <= 1, message="param value length range [0, 1]"),
        question_number=dict(validator=lambda x: 0 < x < INT_32_MAX,
                             message="param value length range (0, 2 ** 31 - 1)"),
