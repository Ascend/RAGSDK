...
class EvalDataGenerator(BaseGenerator):
    ...
    @validate_params(
        max_samples=dict(validator=lambda x: 0 < x < INT_32_MAX, message="param value range (0, 2 ** 31 - 1)"),
        question_number=dict(validator=lambda x: 0 < x < INT_32_MAX, message="param value range (0, 2 ** 31 - 1)"),
        featured_percentage=dict(validator=lambda x: 0 <= x <= 1, message="param value range (0, 1]"),
        llm_threshold_score=dict(validator=lambda x: 0 <= x <= 1, message="param value range (0, 1]")
    )
