...
from mx_rag.utils.common import validate_params, INT_32_MAX, FILE_COUNT_MAX, STR_TYPE_CHECK_TIP, \
    STR_LENGTH_CHECK_1024, check_db_file_limit, validata_list_str, MAX_PATH_WHITE, TEXT_MAX_LEN
...
class KnowledgeStore:
    ...
    @validate_params(
        knowledge_name=dict(validator=lambda x: isinstance(x, str) and len(x) < 1024, message=STR_LENGTH_CHECK_1024),
        doc_name=dict(validator=lambda x: isinstance(x, str) and len(x) < 1024, message=STR_LENGTH_CHECK_1024)
    )
