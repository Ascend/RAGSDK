...
class ClusterCompressorTestCase(unittest.TestCase):
    ...
    def test_init_fail(self):
        with self.assertRaises(ValueError):
            mock_cluster_func = Mock()
            ClusterCompressor(embed="", cluster_func=mock_cluster_func)

        with self.assertRaises(ValueError):
            mock_embed_func = MagicMock(spec=Embeddings)
            ClusterCompressor(embed=mock_embed_func, cluster_func="")

        with self.assertRaises(ValueError):
            mock_embed_func = Mock()
            mock_cluster_func = Mock()
            ClusterCompressor(embed=mock_embed_func, cluster_func=mock_cluster_func, dev_id=-1)

        with self.assertRaises(ValueError):
            mock_embed_func = Mock()
            mock_cluster_func = Mock()
            ClusterCompressor(embed=mock_embed_func, cluster_func=mock_cluster_func, splitter=123)
