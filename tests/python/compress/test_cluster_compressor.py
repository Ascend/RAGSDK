...
from mx_rag.compress import ClusterCompressor


class ClusterCompressorTestCase(unittest.TestCase):
    ...
    def test_success(self, mock_get_similarity):
        ...
        mock_embed_func.return_value = [[random.random() for _ in range(4)] for _ in range(16)]
        mock_cluster_func = Mock()
        mock_cluster_func.return_value = [-1, 0, -1, 0, -1, -1, 1, -1, -1, 0, 0, 1, -1, -1, -1]

        compressor = ClusterCompressor(mock_embed_func, mock_cluster_func)
    ...
    def test_init_fail(self):
        with self.assertRaises(ValueError):
            mock_cluster_func = Mock()
            ClusterCompressor(embed_func="", cluster_func=mock_cluster_func)

        with self.assertRaises(ValueError):
            mock_embed_func = Mock()
            ClusterCompressor(embed_func=mock_embed_func, cluster_func="")

        with self.assertRaises(ValueError):
            mock_embed_func = Mock()
            mock_cluster_func = Mock()
            ClusterCompressor(embed_func=mock_embed_func, cluster_func=mock_cluster_func, dev_id=-1)

        with self.assertRaises(ValueError):
            mock_embed_func = Mock()
            mock_cluster_func = Mock()
            ClusterCompressor(embed_func=mock_embed_func, cluster_func=mock_cluster_func, spliter_func=123)
