...
class TestSummary(unittest.TestCase):
    ...
    def test_summarize(self, summary_mock):
        ...
        res = su.summarize(["aaa", "1b2", "2c3", "ddd"], no_summarize_size=2)
    ...
    def test_split_summary_by_threshold(self):
        su = Summary(llm=self.llm)
        res = su._split_summary_by_threshold(["aaa"], merge_limit=8)
        self.assertSequenceEqual(res, [(0,0)])

        res = su._split_summary_by_threshold(["aaa", "1b2"], merge_limit=8)
        self.assertSequenceEqual(res, [(0, 1)])

        res = su._split_summary_by_threshold(["aaa", "1b2", "2c3"], merge_limit=8)
        self.assertSequenceEqual(res, [(0, 1), (2, 2)])

        res = su._split_summary_by_threshold(["aaa", "1b2", "2c3", "ddd", "67"], merge_limit=8)
    ...
    def test_merge_text_summarize(self, summary_mock):
        ...
        res = su.merge_text_summarize(["aaa"], merge_limit=8, no_summarize_size=4)
        self.assertSequenceEqual(res, "aaa")

        res = su.merge_text_summarize(["aaa"], merge_limit=8, no_summarize_size=2)
        self.assertSequenceEqual(res, "aa")

        res = su.merge_text_summarize(["aa", "eb2", "hc34"], merge_limit=8, no_summarize_size=2)
        self.assertSequenceEqual(res, "aa\n\ne\n\nh")