...
class TestDocumentApp(unittest.TestCase):
    def test_document_app(self):
        with patch("mx_rag.vectorstore.MindFAISS") as MindFAISS:
            ...
            doc_app.sql_db.check_document_exist = MagicMock(return_value=True)
            doc_app.delete_file(["demo.docx"])
            doc_app.sql_db.check_document_exist.reset_mock()
            doc_app.sql_db.check_document_exist = MagicMock(return_value=False)
